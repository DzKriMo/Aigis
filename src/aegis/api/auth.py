...
def issue_token():
    ...
    if tenant is None:
        ...
        s.commit()
        s.refresh(tenant)
    key = s.query(ApiKey).filter_by(key=settings.aegis_api_key).first()
    if key is None:
        key = ApiKey(tenant_id=tenant.id, key=settings.aegis_api_key, active=True)
        s.add(key)
        s.commit()
    s.close()
    token = create_token(tenant_id=tenant.id, api_key=settings.aegis_api_key)
